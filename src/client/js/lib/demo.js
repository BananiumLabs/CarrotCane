...
$().ready(function() {
    ...
    setTimeout(function() {
        if (coords.innerHTML.trim() === 'Current Coordinates:')
            demo.errorNotification();
        else
            demo.successNotification();
    }, 3000);
...
demo = {
    ...
    errorNotification: function(from, align) {
        var msg = `ERROR: CarrotCane not detected! <a href="connect.html" class="notification-link" style="color: #e0e0e0">Click here to connect!</a>`;
        if(cookies.getCookie('server').length > 0)
            msg = `ERROR: Could not connect to server: ` + cookies.getCookie('server');
        $.notify({
